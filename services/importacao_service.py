...
async def processar_planilha_clientes_e_pedidos(
    ...
    if not evento:
        raise HTTPException(
            status_code=404,
            detail=f"O evento {evento_id} não foi encontrado ou foi removido.",
        )
    ...
    for (
        ...
        pedidos_no_banco.add(
            (
                ...
                (
                    str(metodo_pagamento_db).strip().lower()
                    if metodo_pagamento_db
                    else None
                ),
            )
        )
    ...
    col_telefone = (
        "telefone_do_comprador" if "telefone_do_comprador" in df.columns else "telefone"
    )
    col_nasc = (
        "data_de_nascimento"
    ...
    col_status_pedido = (
        "status_do_pedido" if "status_do_pedido" in df.columns else "status_pedido"
    )
    col_status_ingresso = (
        "status_do_ingresso"
        if "status_do_ingresso" in df.columns
        else "status_ingresso"
    )
    col_valor_lote = "valor_do_lote" if "valor_do_lote" in df.columns else "valor"
    col_canal_venda = (
        "canal_de_venda" if "canal_de_venda" in df.columns else "canal_venda"
    )
    col_metodo_pagamento = (
        "metodo_de_pagamento"
        if "metodo_de_pagamento" in df.columns
    ...
    df["_data_nasc_limpa"] = (
        pd.to_datetime(df[col_nasc], errors="coerce").dt.date
        if col_nasc in df.columns
        else None
    )
    df["_data_venda_limpa"] = (
        pd.to_datetime(df[col_data_venda], errors="coerce")
        if col_data_venda in df.columns
        else None
    )
    df["_id_pedido_limpo"] = (
        df[col_id_pedido].apply(limpar_id_pedido)
        if col_id_pedido in df.columns
        else None
    )
    ...
    if len(cpfs_unicos) > 0:
        try:
            rows = (
                db.query(Cliente.cpf, Cliente.id)
                .filter(Cliente.cpf.in_(list(cpfs_unicos)))
                .all()
            )
    ...
    if cpfs_novos:
        df_novos_clientes = df_validos[
            df_validos["_cpf_limpo"].isin(cpfs_novos)
        ].drop_duplicates(subset=["_cpf_limpo"])
        novos_clientes_objs = []
        ...
        for _, row in df_novos_clientes.iterrows():
            ...
            novo_cliente = Cliente(
                ...
                data_nascimento=(
                    row["_data_nasc_limpa"]
                    if pd.notna(row["_data_nasc_limpa"])
                    else None
                ),
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Erro ao salvar lote de novos clientes: {str(e)}",
            )
        ...
        for index, row in enumerate(linhas_planilha):
            if index % 50 == 0:
                ...
                if await request.is_disconnected():
                    print(
                        f"❌ [CANCELADO] O usuário cancelou a importação no front-end na linha {index}!"
                    )
                    db.rollback()
                    return {
                        "status": "cancelado",
            ...
            canal_venda_val = (
                row.get(col_canal_venda) if col_canal_venda in row else None
            )
            metodo_pagamento_val = (
                row.get(col_metodo_pagamento) if col_metodo_pagamento in row else None
            )
            ...
            valor_lote_key = (
                round(float(valor_lote_val), 2) if pd.notna(valor_lote_val) else 0.0
            )
            canal_venda_key = (
                str(canal_venda_val).strip().lower()
                if pd.notna(canal_venda_val)
                else None
            )
            metodo_pagamento_key = (
                str(metodo_pagamento_val).strip().lower()
                if pd.notna(metodo_pagamento_val)
                else None
            )
            ...
            try:
                status_pedido_val = (
                    row.get(col_status_pedido) if col_status_pedido in row else None
                )
                status_ingresso_val = (
                    row.get(col_status_ingresso) if col_status_ingresso in row else None
                )
                ...
                novo_pedido = Pedido(
                    ...
                    evento_id=evento_id,
                    data_venda=(
                        data_venda_val.to_pydatetime()
                        if pd.notna(data_venda_val)
                        else datetime.now()
                    ),
                    status_pedido=(
                        str(status_pedido_val).strip()
                        if pd.notna(status_pedido_val)
                        else None
                    ),
                    status_ingresso=(
                        str(status_ingresso_val).strip()
                        if pd.notna(status_ingresso_val)
                        else None
                    ),
                    lote=str(lote_val).strip() if pd.notna(lote_val) else None,
                    valor_lote=valor_lote_val if pd.notna(valor_lote_val) else 0.0,
                    canal_venda=(
                        str(canal_venda_val).strip()
                        if pd.notna(canal_venda_val)
                        else None
                    ),
                    metodo_pagamento=(
                        str(metodo_pagamento_val).strip()
                        if pd.notna(metodo_pagamento_val)
                        else None
                    ),
                    transferido=(
                        str(transferido_val).strip()
                        if pd.notna(transferido_val)
                        else None
                    ),
                ...
                raise HTTPException(
                    status_code=400,
                    detail=f"Erro na linha {index} ao salvar pedido: {str(e)}",
                )

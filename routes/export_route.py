...
def exportar_ingressos_endpoint(payload: ExportarIngressosRequest):
    try:
        ...
        if not cpf or not senha:
            raise HTTPException(
                status_code=500,
                detail="Credenciais CHEERS_CPF ou CHEERS_SENHA não configuradas no ambiente.",
            )

        caminho_storage = exportar_ingressos(
            cpf=cpf, senha=senha, evento=payload.evento
        )
        ...
        return {
            ...
            "mensagem": f"Arquivo '{nome_arquivo}' exportado com sucesso.",
        }
...
def baixar_arquivo_exportado(payload: BaixarArquivoExportadoRequest):
    ...
    try:
        ...
        if not response:
            raise HTTPException(
                status_code=404,
                detail=f"Arquivo '{payload.nome_arquivo}' não encontrado no Supabase.",
            )
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": f"attachment; filename={payload.nome_arquivo}"
            },
        )
